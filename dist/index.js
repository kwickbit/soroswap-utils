...
var testnetCacheFile = join(cacheDirectory, "testnet-assets.json");
var daysToCache = 30;
var hoursPerDay = 24;
...
var callPoolContract = async (poolAddress, sorobanFunctionName) => {
  ...
  return scValToNative(xdr.ScVal.fromXDR(firstResult.xdr, "base64"));
};
...
var getLiquidityPoolCount = async () => {
  const config2 = getConfig();
  const server = new SorobanRpc.Server(config2.rpc.url);
  ...
  return scValToNative(xdr.ScVal.scvMap(storage)).TotalPairs;
};
